...
import com.ava.frame.mongo.util.RegexUtils;
...
public class Mongo extends MongoQuery {
    ...
    private Mongo(String source) {
        super();
        mongoTemplate = SpringApplicationContext.getBean(source);
    }

    private Mongo() {
        this.mongoTemplate = SpringApplicationContext.getBean("mongoTemplate");
    }

    public static Mongo build() {
        return new Mongo();
    }
    public static Mongo build(String source) {
        return new Mongo(source);
    }
    public List list(Class clazz) {
        return mongoTemplate.find(query, clazz, collectionName(clazz));
    }
    ...
    public List<GeoResult> geoFind(Double lat, Double lng, Class clazz, String collectionName) {
        ...
        GeoResults geoResults = mongoTemplate.geoNear(nearQuery, clazz, collectionName);
    ...
    public WriteResult upsert(IMongoUpdate update, String collectionName) {
        ...
        return mongoTemplate.upsert(query, update1, collectionName);
    }

    public WriteResult updateMulti(IMongoUpdate update, String collectionName) {
        Update update1 = new Update();
        update.update(update1);
        return mongoTemplate.updateMulti(query, update1, collectionName);
    }

    public void updateFirst(IMongoUpdate update, String collectionName) {
        Update update1 = new Update();
        update.update(update1);
        mongoTemplate.updateFirst(query, update1, collectionName);
    }

    public void updateFirst(IMongoUpdate update, Object obj) {
        Update update1 = new Update();
        update.update(update1);
        mongoTemplate.updateFirst(query, update1, collectionName(obj));
    }
    ...
    public <T> T findAndUpdate(Class clazz, String collectionName, IMongoUpdate update) {
        Update update1 = new Update();
        update.update(update1);
        return (T) mongoTemplate.findAndModify(query, update1, clazz, collectionName);
    }
...
//            AggregationResults aggregationResults = mongoTemplate.aggregate(Aggregation.newAggregation(geoNearOperation), collectionName(clazz), HashMap.class);
            GeoResults geoResults = mongoTemplate.geoNear(nearQuery, clazz, collectionName(clazz));
            return geoResults.;
        } catch (Exception e) {
    ...
    private abstract class MyAggregation<T extends Number> {
        public T handler(String key, String collectionName, String... group) {
            ...
            AggregationResults aggregationResults = mongoTemplate.aggregate(Aggregation.newAggregation(new MatchOperation(criteria), nextGroupOperation(groupOperation, key).as(valueKey)), collectionName, HashMap.class);
    ...
    public <T> T findOne(Class clazz) {
        return (T) mongoTemplate.findOne(query, clazz, collectionName(clazz));
    }
    ...
    public void ensureIndex(String name, Order order, String collectionName, boolean unique) {
        DBObject dbObject = new BasicDBObject();
        dbObject.put(name, order.getOrderValue());
        mongoTemplate.getCollection(collectionName).createIndex(dbObject, name + "_" + order.getOrderValue(), unique);
    }
    ...
    public void ensureIndex(Map<String, Order> keys, String collectionName, boolean unique) {
        ...
        mongoTemplate.getCollection(collectionName).createIndex(dbObject, name.substring(0, name.length() - 1), unique);
    }
